...
def take_move_card(game_id,player_id):
    card_info = repo.create_card(random.randint(0,6),"movement",game_id,player_id)
    return card_info
...
def take_figures_card(game_id,player_id):
    ...
    for i in range(AMOUNT_FIGURE_FOR_PLAYER):
               card_info.append(repo.create_card(random.randint(0, 12), "figure", game_id, player_id, "Drawn"))
