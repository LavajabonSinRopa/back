...
from entities.cards.figure_cards import print_figure, normalize_card, rotate_card, figure_matches, figure_card_types_hard, figure_card_types_easy

def test_find_figures_that_exist():
    for card in figure_card_types_hard:
        for rotation in range(4):
            card = rotate_card(card)
            assert figure_matches(card)
...
def test_find_figures_that_dont_exist():
    card = [[0,0]]*4
    assert not figure_matches(card)
    card = [[0,0]]*3
    assert not figure_matches(card)
    card = [[0,0]]*6
    assert not figure_matches(card)
...
def test_rotate_card():
    ...
    n = len(figura)
    exists = figure_matches(figura)
    for rotation in range(4):
        figura = rotate_card(figura)
        assert len(figura) == n
        assert figure_matches(figura) == exists
        print_figure(figura)
